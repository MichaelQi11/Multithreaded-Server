...
void *ServerEcho(void *args)
{
    ...
    char requestReceive[2*COM_BUFF_SIZE];

    read(clientFileDescriptor, requestReceive, 2*COM_BUFF_SIZE);
...
int main(int argc, char* argv[])
{
    ...
    int arraySize = atoi(argv[1]); //strtol(argv[1], NULL, 10);
    theArray = (char**)malloc(arraySize*sizeof(char *));
    lock = malloc(arraySize*sizeof(pthread_mutex_t));
    
    for(int a = 0; a < arraySize; a++) {
        theArray[a] = (char*)malloc(COM_BUFF_SIZE * sizeof(char));        
